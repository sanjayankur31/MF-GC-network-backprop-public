...
def generate_grc_positions(density, diam):
    """Generates uniformly distributed granule positions of fixed density
    within ball of diameter diam"""
...
def generate_glom_positions(p_num_glom, glom_density, dx, dy, dz, diam):
    ...
    big_diam = diam * 3
    # embed in big cube to avoid boundary effects
    ...
    glom_mf_id = np.zeros(
        int(num_glom.sum()), int
    )  # vector that indexes which mossy fiber is associated with each glomeruli
    ...
    glom_pos = glom_pos[which_glom_in_ball, :]
    glom_mf_id = glom_mf_id[which_glom_in_ball]
    glom_mf_id = renumber(glom_mf_id)
...
def closest_allowed_grc(attached_grcs, grc_pos, this_glom_pos, conn_mat, d, dlen):
    """Find granule cell that is closest to being dlen away from a given
    glomeruli that is also not connected to that mossy fiber or that has more
    than d connections
    """
...
def check_valid_connectivity(conn_mat, ddist, glom_mf_id, d):
    ...
    for mf in range(N_mf):
        ...
        assert np.all(conn_mat[gloms, :].sum(axis=0) <= 1), (
            "Mossy fiber " + str(mf) + " has multiple connections to same granule cell."
        )
    assert np.all(
        conn_mat.sum(axis=1) - ddist == 0
    ), "Glomeruli degree distribution does not match desired distribution."
    assert np.all(conn_mat.sum(axis=0) == d), (
        "Granule cells do not all have " + str(d) + " dendrites."
    )
...
def optswap(glom_incomplete, grcs_incomplete, grcs_swappable, grc_pos, glom_pos, conn_mat, dlen):
    ...
    assert (
        conn_mat[opt_glom_ii, opt_grc_B] == 1
    ), "Something is wrong with function optswap: glom ii is not connected to grc B."
    assert (
        conn_mat[glom_incomplete, opt_grc_A] == 1
    ), "Something is wrong with function optswap: glom i is not connected to grc A."
    assert (
        conn_mat[opt_glom_ii, opt_grc_A] == 0
    ), "Something is wrong with function optswap: glom ii is connected to grc A."
...
def alg_connections(grc_pos, glom_pos, glom_mf_id, d, dlen):
    ...
    for conn in range(1, ddist.max() + 1):
        for gl in range(N_glom):
            if conn <= ddist[gl]:
                ...
                this_grc = closest_allowed_grc(
                    attached_grcs, grc_pos, glom_pos[gl, :], conn_mat, d, dlen
                )
                if this_grc >= 0:
    ...
    if not np.all(conn_mat.sum(axis=1) - ddist == 0):
        ...
        conn_mat = shuffle_conns(
            grc_pos, glom_pos, d, dlen, gloms_incomplete, conn_mat, ddist
        )
...
glom_pos, glom_mf_id = generate_glom_positions(
    p_num_glom, glom_density, glom_dx, glom_dy, glom_dz, diam
)
N_glom = glom_pos.shape[0]
N_grc = grc_pos.shape[0]
    ...
    for gl in range(N_glom):
        for grc in range(N_grc):
            if conn_mat[gl, grc] == 1:
                ...
                ix = ix + 1
    # Save to file
    with open("GCLconnectivity_" + str(d) + ".pkl", "wb") as file:
        p = {
            "conn_mat": conn_mat,
            "ddist": ddist,
            "dlens": dlens,
